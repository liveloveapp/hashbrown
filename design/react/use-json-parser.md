...
function Demo() {
  const { value, error, parserState, parseChunk, reset } = useJsonParser(schema);
...
- React: `useJsonParser(schema?: s.HashbrownType)`
...
`useJsonParser` keeps parser + schema state in React state and updates with the existing core functions. The hook only returns public types (no `ɵ` types), and is pure:

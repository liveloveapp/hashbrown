...
# injectImperativeJsonParser (Angular)
...
- Provide a public Angular signal factory (`injectImperativeJsonParser`) that:
...
export class DemoComponent {
  parser = injectImperativeJsonParser(
    s.object('root', {
...
- Angular: `injectImperativeJsonParser(schema?: s.HashbrownType)`
...
`injectImperativeJsonParser` creates signals for `parserState`, `value`, `error`, and maintains schema cache/last value internally. It exposes imperative methods `parseChunk` and `reset`, and uses core `createParserState/parseChunk/fromJsonAst` for updates.

...
Even with good UX the form still requires **five** separate inputs, data fetching logic, and validation messages. Imagine localising all of that and making it accessible!
...
- Describe each case with its own schema
- _Tag_ each case using `s.literal()` so that you can _discriminate_ on the result later
- Use `s.infer` to produce TypeScript types from our schema
- Join each branch using `s.anyOf`
...
We are @hashbrownai/react!useTool:function to create a tool the LLM can invoke at runtime. The LLM
will be able to infer the correct category from whatever language the user has written their response in.

Key Points:

- Use @hashbrownai/react!useTool:function to create a tool the LLM can invoke at runtime.
- Tools are passed an abort signal that you can use to clean up tool calls when a completion is cancelled (in this case, as a result of new user input).
- Map each cateogry into the smallest data element possible. Avoid providing responses directly as that might fill the completion's context with unnecessary information.

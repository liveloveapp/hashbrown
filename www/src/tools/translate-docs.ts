...
async function main() {
  ...
  const mdFiles = await readdir('www/content/docs', { recursive: true });
  ...
  for (const file of mdFiles.filter((p) => p.endsWith('.md'))) {
    ...
    const srcPath = join('www/content/docs', file);
    const srcMarkdown = await readFile(srcPath, 'utf8');

    const targetRelPath = file.replace(FROM, TO);
    const targetPath = join('www/content/docs', targetRelPath);
    ...
    try {
      const { markdown, observations } = await translateDocs({
        ...
        relPath: relative('www/content/docs', srcPath),
      });

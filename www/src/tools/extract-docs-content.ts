...
// This file is www/src/tools/extract-docs-content.ts.
...
async function writeFinalizedApiReport() {
  ...
  const minimizedReportPath = path.join(
    MONOREPO_ROOT,
    './www/content/reference/api-report.min.json',
  );

  const baseReferencePath = path.join(MONOREPO_ROOT, './www/content/reference');
  ...
  for (const packageName of report.packageNames) {
    ...
    for (const symbolName of packageReport.symbolNames) {
      ...
      const basePath = path.join(MONOREPO_ROOT, `./www/content/reference`);
      ...
      const symbolPath = path.join(
        MONOREPO_ROOT,
        `./www/content/reference/${packagePath.join('/')}/${symbol.name}.json`,
      );

...
import {
  ...
  STEPS,
  THREADPLANE_URL,
} from '../../src/components/home/home.content';
...
test('links a capability to the docs of the selected framework', () => {
  const capability = CAPABILITIES[0];

  const result = capabilityDocsUrl('react', capability);

  expect(result).toBe('/docs/react/concept/components');
});

---
title: 'Upgrade to v0.6: Hashbrown Angular Docs'
meta:
  - name: description
    content: 'Upgrade a Hashbrown Angular app from v0.5 to v0.6: AG-UI server endpoints, server-side model selection, structured output, thread persistence, core runtime renames, and a migration prompt for AI coding assistants.'
---
...
There is no compatibility export for the binary frame API, so migrate the server route and the client URL together. See the platform page for your provider, such as [OpenAI](/docs/angular/platform/openai), for Fastify, Next.js and other server shapes.
...
<hb-code-example header="app.config.ts">
...
See [Persist and Resume Threads](/docs/angular/recipes/threads) for a complete example.
...
See [Custom Provider](/docs/angular/platform/custom) for the event lifecycle.

---

## Magic Text and JSON Parser Internals

Hashbrown's Markdown and JSON parsers now come from [Cacheplane](https://github.com/cacheplane/cacheplane). The `MagicText` component, `injectJsonParser` and `injectImperativeJsonParser` work as before. What changed is the low-level surface that `@hashbrownai/core` used to export.
...
`@hashbrownai/writer` is no longer published, and `WriterKnownModelIds` was removed from core. Move to another supported provider, or wrap Writer's SDK in a [custom provider](/docs/angular/platform/custom).
...
- [Interrupt and resume](/docs/angular/concept/interrupt-and-resume), for human-in-the-loop approvals from AG-UI servers.
- [Shared agent state](/docs/angular/concept/shared-agent-state), synchronized with your server over AG-UI.
- [Server-executed tool calls](/docs/angular/concept/functions#server-executed-tool-calls), which surface on assistant messages as `serverToolCalls`, with arguments that resolve while the server streams them.
...
<hb-next-steps>
  <hb-next-step link="/docs/angular/migrations/v0-5">
    <div>
      <hb-code />
    </div>
    <div>
  ...
  <hb-next-step link="/docs/angular/platform/openai">
    <div>
      <hb-code />
    </div>
    <div>

...
10. [Replace the Writer provider.](#writer-provider) Only if you used `@hashbrownai/writer`.
...
`@hashbrownai/writer` is no longer published, and `WriterKnownModelIds` was removed from core. Move to another supported provider, or wrap Writer's SDK in a [custom provider](/docs/react/platform/custom).

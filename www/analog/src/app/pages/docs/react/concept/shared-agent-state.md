---
title: 'Shared Agent State: Hashbrown React Docs'
meta:
  - name: description
    content: 'Synchronize JSON-compatible application state with an AG-UI agent in React.'
---
...
Pass an initial `state` value when you create a runtime-backed chat or completion hook. This works with `useChat`, `useStructuredChat`, `useCompletion`, `useStructuredCompletion`, `useUiChat`, and `useUiCompletion`.

<hb-code-example header="account-assistant.tsx">
...
The hook subscribes to runtime state. When the agent updates the shared value, React rerenders with the latest value in `chat.state`.

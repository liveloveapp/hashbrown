...
> Historical record of the migration from Analog. Paths below (`www/next`,
> `www/analog`, `www-next`) predate the cutover: the site now lives in `www/`
> (Nx project `www`), with content in `www/content`, and `www/analog` is gone.

...
An [interrupt](/docs/angular/concept/interrupt-and-resume) also commits the current state and message checkpoint. While awaiting answers, you can edit state with `setState()`. Resume captures that latest committed state and locks writes before scheduling its request.

...
(async () => {
  ...
  const responseSchema = s.object('UPDATE_FORM_VALUE action payload', {
    name: s.string('The name of the form field in the form object in Redux.'),
    value: s.anyOf([
      s.string('Used for text fields'),
      s.number('Used for number fields and select fields'),
      s.array(
        'Used for multi-select fields and radio groups',
        s.anyOf([
          s.string('Used for string enums'),
          s.number('Used for numeric enums and DB IDs'),
        ]),
      ),
    ]),
  });

...
import { SampleScriptService } from './sample-script.service';
...
export class SampleGatewayService {
  ...
  private onMessage = (event: MessageEvent) => {
    if (!this.allowedParentOrigin || event.origin !== this.allowedParentOrigin)
      return;
    const data = event.data as SampleCommand | unknown;
    if (!data || typeof (data as any).type !== 'string') return;

    switch ((data as any).type) {
      case 'sample/start':
        if ((data as any).payload?.speed != null)
          this.engine.setSpeed((data as any).payload.speed);
        this.engine.start();
        break;
      case 'sample/pause':
        this.engine.pause();
      ...
      case 'sample/restart':
        this.engine.restart({
          hardReset: (data as any).payload?.hardReset,
          speed: (data as any).payload?.speed,
        });
        break;
      case 'sample/set-speed':
        this.engine.setSpeed((data as any).payload?.speed ?? 1);
        break;
      default:
        break;
    }

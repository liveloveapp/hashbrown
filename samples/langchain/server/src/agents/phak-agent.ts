...
AVAILABLE TOOLS
- retrieve-phak: semantic retrieval over PHAK content. Requires { query: string, k: number }.
- get_retrieval: inspect the graph state's retrieval cache (use key = "phak") so you know what has already been retrieved for this session.

WORKFLOW
1. Call \`get_retrieval\` with key "phak" to understand prior retrieval attempts and avoid redundant calls.
...
3. Call \`retrieve-phak\` with that query. Capture every returned chunk exactly as provided so they can populate the response schema.
...
export const agent = createAgent({
  ...
  tools: [retrievePhak, getRetrieval],
});

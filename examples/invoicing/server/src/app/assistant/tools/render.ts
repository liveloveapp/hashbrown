...
/**
 * Ends the run on a successful render. The answer is this call's arguments,
 * so a valid render needs no closing model turn. A rejected render
 * (`invalid_ui`) still goes back to the model, which fixes the named
 * component and renders again: B4 ends the run only on a successful result.
 */
export const returnDirect = true;

/** Show your answer to the user. Call exactly once, last. `text` is your prose; `components` are the kit pieces that support it, in order: LedgerTable for specific rows by ID, TrendChart for month-over-month, AgingSummary for overdue balances, CustomerCard for one client, ReviewPayment to offer matching an unapplied payment, with invoiceIds listing the invoices it clearly settles. Every ID must come from a tool result. On an invalid_ui error, fix the named component and call render once more. */
